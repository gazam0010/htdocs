                ...
                if (isset($_GET['search'])) {
                    ...
                    $sno = 1;
                            while ($fetch = $output->fetch_assoc()) {
                                echo '<table width="100%" border="1px" id="table-output">';
                                $id = $fetch['id'];
                                $fname = $fetch['fname'];
                                $email = $fetch['email'];
                                $mobile = $fetch['mobile'];
...
$sno++;
                                ...
                                echo '<button type="button" id="userControlModal" onclick=getUserControlModal("p'.$sno.'") class="w3-button w3-black">Access</button>';
                                echo '</td>';
                                echo '</form>';
                                echo '</tr>';
                            ...
                            <div id="p'.$sno.'" class="w3-modal w3-animate-opacity w3-padding-64">
                              <div class="w3-modal-content w3-card-4">
                                 <header class="w3-container w3-teal">
                                    <span onclick=closeUserControlModal("p'.$sno.'") class="w3-button w3-large w3-display-topright">&times;</span>
                                    <h2>User Control</h2>
                                 </header>';
                            include('../alert.php');
                            echo '<div class="w3-container">';
...
mysqli_close($db1);
    ...
    <?php endif; ?>
    <script>
        function getUserControlModal(div_id) {
            document.getElementById(div_id).style.display = 'block';
        }

        function closeUserControlModal(div_id) {
